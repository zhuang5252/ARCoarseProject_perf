...
from air_track.detector.utils.calculate_metrics import calculate_metrics_with_distance
...
def detector_out_2_classifier_input(cfg, detected_objects, pred_feature, down_scale):
    """检测器的输出转换为分类器的输入"""
...
def test(cfg_inference):
    ...
    for i, data in enumerate(valid_dl):
        ...
        if cfg_inference['use_second_classify']:
            classify_predictor.normalize = True
            # 按照cfg['second_classifier_params']['save_data_size']裁剪特征图和设置标签
            mask_pred_feature = prediction[-1][0].float().detach()
            # 检出目标
            if detected_objects:
                classify_input_data = detector_out_2_classifier_input(classify_cfg, detected_objects,
                                                                      mask_pred_feature,
                                                                      down_scale=model_params['down_scale'])
