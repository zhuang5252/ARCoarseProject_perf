...
2. 根据csv中的Channel和img_name字段构建图片路径
3. 复制图片到目标目录并重命名
...
def find_image_path(src_root, channel, img_name):
    """根据channel和img_name查找图片路径"""
    # 可能的图片扩展名
    image_extensions = ['.jpg', '.jpeg', '.png', '.bmp', '.tiff']

    # 构建基础路径
    base_path = Path(src_root) / "Images" / channel
...
def process_subdirectory(sub_dir, dst_img_dir, dst_label_dir, counter, csv_writer=None):
    ...
    for i, row in enumerate(csv_data, 1):
        ...
        if not channel or not img_name:
            print(f"警告: 第 {i} 行缺少Channel或img_name字段，跳过")
            continue

        # 查找图片
        img_path = find_image_path(sub_dir, channel, img_name)

        if not img_path or not img_path.exists():
            print(f"警告: 找不到图片 {sub_dir}/Images/{channel}/{img_name}，跳过")
            continue
...
def main():
    ...
    parser.add_argument('--src_roots', nargs='+', default=['/home/csz_changsha/data/AR_2410_2511/AR_2511_test_data'],
                        help='源数据根目录路径列表，例如: ./scne31_ch3 ./scne31_ch4')
    parser.add_argument('--dst', type=str, default='/home/csz_changsha/data/AR_2410_2511/AR_2511_test_data_merge',
