...
from air_track.utils import common_utils, reprod_init, col_data_save_csv, round_up_to_nearest_power_of_two, \
    is_bbox_overlap, is_center_point_in_boxes
...
def get_data_and_label(cfg, detected_objects, orig_img, cur_labels, scale_w=1, scale_h=1):
    ...
    for detected_object in detected_objects:
        ...
        if second_classifier_params['save_data_size'] != 'None':
            ...
            # 确定长边
            max_len = max(detected_object['w'] * scale_w, detected_object['h'] * scale_h)
            # 最小尺寸在检测环节中的target_min_size规定了，这里进行双重约束
            img_size_w = max(max_len * 2, target_min_size)
            img_size_h = max(max_len * 2, target_min_size)
            # 向上取整到最近的2的倍数
        ...
        bbox_xywh = [detected_object['cx'] * scale_w,
                     detected_object['cy'] * scale_h, img_size_w, img_size_h]
        ...
        for cur_label in cur_labels[0]:
            ...
            # 这里的min_w和h是为了防止标签框为一个点，导致无法判断是否是正样本
            label_xyxy = cxcywhn2xyxy(label_xywhn, orig_w, orig_h, min_w=1, min_h=1)
            # 检查边界条件并调整
            label_xyxy = check_boundary(label_xyxy, img_size_w, img_size_h, orig_w, orig_h)

            # 判断是否是正样本
            # pos_flag = is_center_point_in_boxes(cx, cy, label_xyxy)  # 判断点(x,y)是否在标签box内
            # 判断预测框与标签框的交集面积是否超过标签框面积的阈值
            pos_flag = is_bbox_overlap(bbox_xyxy, label_xyxy,
                                       threshold=cfg['second_classifier_params']['pos_overlap_threshold'])
            if pos_flag:
        ...
        # RGB格式的图像（最后一个维度为3）
        if len(orig_img.shape) == 4 and orig_img.shape[-1] == 3:
            img = orig_img[0, y1:y2, x1:x2, -3:].cpu().numpy()
        elif len(orig_img.shape) == 4 and orig_img.shape[-1] == 1:
            img = orig_img[0, y1:y2, x1:x2, -1:].cpu().numpy()  # img为(b, h, w, c)
        elif len(orig_img.shape) == 3:
            img = orig_img[0, y1:y2, x1:x2].cpu().numpy()  # img为(b, h, w)
            img = np.expand_dims(img, axis=-1)  # img转换为(b, h, w)
        else:
    ...
    '''如果检测器检测出来的目标不存在真目标，则从标签中找到真目标的位置，并把真目标的img和label插入到列表开头'''
    if not pos_exist:
        for cur_label in cur_labels[0]:
            cls_name = cur_label['cls_name'][0]

            # 确定长边
            max_len = max(cur_label['w'].item() * scale_w, cur_label['h'].item() * scale_h)
            # 由于标签信息没有约束最小尺寸，在这里进行target_min_size * scale_w 约束
            img_size_w = max(max_len * 2, target_min_size)
            img_size_h = max(max_len * 2, target_min_size)
            # 向上取整到最近的2的倍数
            img_size_w = round_up_to_nearest_power_of_two(img_size_w)
            ...
            # 获取框信息
            label_xywhn = [cur_label['cx'].item(), cur_label['cy'].item(), cur_label['w'].item(), cur_label['h'].item()]
            label_xyxy = cxcywhn2xyxy(label_xywhn, orig_w, orig_h, min_w=img_size_w, min_h=img_size_h)
...
def write_img(data_idx, img_lists, label_lists, save_path, save_img_flag=True, max_pixel=255, use_gray=False):
    ...
    for i in range(data_idx, len(label_lists)):
        ...
        if max_pixel == bit_depth_standards['uint8']:
            img = img_lists[i - data_idx].astype(np.uint8)
        elif max_pixel == bit_depth_standards['uint16']:
            img = img_lists[i - data_idx].astype(np.uint16)
        else:
            raise ValueError("Saving image does not support types other than 'unint8' and 'unint16'")
...
if __name__ == "__main__":
    ...
    predict_yaml = os.path.join(script_dir, 'config/predict_AR2511.yaml')
    ...
    # stage_list = [cfg_predict['stage_train'], cfg_predict['stage_valid']]
