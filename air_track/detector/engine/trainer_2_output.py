...
class Trainer(BaseTrainer):
    ...
    def train(self, epoch):
            ...
            with torch.set_grad_enabled(True):
                ...
                if self.is_rank0 and self.logger and (index % 10 == 0):
                    self.logger.info(
                        ...
                        f'mask_loss={res_loss_item["mask"]}, size_loss={res_loss_item["size"]}, '
        ...
        if self.distributed:
            # 同步所有进程的损失
            for name in ['train_mask_loss', 'train_size_loss', 'train_sim_loss', 'train_total_loss']:
    ...
    def valid(self, epoch):
        ...
        if self.distributed:
            # 同步所有进程的损失
            for name in ['test_mask_loss', 'test_size_loss', 'test_sim_loss', 'test_total_loss']:
                tensor = torch.tensor(locals()[name], device=self.device)
                dist.all_reduce(tensor, op=dist.ReduceOp.SUM)
                locals()[name] = tensor.item() / self.world_size
