...
from air_track.aligner.model import resnet34_return_tr
...
class Trainer:
    ...
    def set_model(self):
        ...
        self.model: nn.Module = resnet34_return_tr.__dict__[model_params['model_cls']](cfg=model_params,
    ...
    def train(self, epoch):
        ...
        for data in self.train_dl:
            with torch.set_grad_enabled(True):
                ...
                transform_label = data['tr_transform'].to(self.device)

                cur_points = data['cur_points'].to(self.device)
                prev_points = data['prev_points'].to(self.device)

                self.optimizer.zero_grad()

                with torch.cuda.amp.autocast(False):  # 混合精度
                    transform_pred = self.model(prev_frame, cur_frame)
                    # 计算损失
                    loss, loss_item = self.calc_loss(transform_pred, transform_label)
                ...
                if index % 10 == 0:
                    self.logger.info(
                        'Epoch:{:2d} 第[{}/{}]次迭代, train_loss: {}\t '.format(epoch, index, num_batches, loss_item))
                index += 1
    ...
    def valid(self, epoch):
        ...
        for data in self.valid_dl:
            with torch.set_grad_enabled(False):
                ...
                if index % 10 == 0:
                    self.logger.info(
                        'Epoch:{:2d} 第[{}/{}]次迭代, test_loss: {}\t '.format(epoch, index, num_batches, loss_item))
                index += 1
        ...
        # 保存最新的模型
        self.logger.info('epoch = {:2d}, saving model to latest.pt'.format(epoch))
        self.save_model('latest.pt', epoch)

        # 保存第一个epoch模型
        if epoch == 1:
            for yaml_file in self.yaml_list:
                shutil.copy(yaml_file, os.path.join(self.model_save_path, os.path.basename(yaml_file)))
            self.logger.info('epoch = {:2d}, saving model to first.pt'.format(epoch))
            self.save_model('first.pt', epoch)
