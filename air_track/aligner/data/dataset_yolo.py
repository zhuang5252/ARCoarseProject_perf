...
class AlignDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        for part in self.parts:
            data_dir = os.path.join(self.cfg['data_dir'], part, self.cfg['img_folder'])
            img_paths = self.load_dataset(data_dir)
            self.img_paths.extend(img_paths)

        if len(self.img_paths) > sample_nums:
            self.img_paths = self.img_paths[:sample_nums]
    ...
    def load_dataset(self, data_dir):
        ...
        for img_name in os.listdir(data_dir):
            if img_name.endswith(self.cfg['img_format']):
