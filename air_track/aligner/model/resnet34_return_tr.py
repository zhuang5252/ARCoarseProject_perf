...
class TrEstimatorTsm(nn.Module):
    def __init__(self, cfg, pretrained=True):
        ...
        self.feature_fusion = nn.Sequential(
            nn.Conv2d(out_ch * 2, 512, 3, padding=1),  # 双帧特征融合‌:ml-citation{ref="2" data="citationList"}
            nn.BatchNorm2d(512),
        ...
        # 参数初始化‌:ml-citation{ref="3" data="citationList"}
    ...
    def forward(self, prev_frame, cur_frame):
        ...
        # 特征提取‌:ml-citation{ref="2" data="citationList"}
        features = self.base_model(combined)[-1]  # 取最后层特征
        B = prev_frame.size(0)
        prev_feat, cur_feat = features[:B], features[B:]  # 分割前后帧特征

        # 特征融合
        fused = torch.cat([prev_feat, cur_feat], dim=1)  # 通道维度拼接‌:ml-citation{ref="2" data="citationList"}
