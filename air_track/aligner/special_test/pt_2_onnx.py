...
if __name__ == '__main__':
    path = '/home/csz_changsha/PycharmProjects/github/zhuang5252/AirTrack_all/AirTrack_multi_frame/model_saved/TrEstimatorTsm/aligner_return_dx_dy_angle'
    checkpoint_path = os.path.join(path, 'minimum_loss_111.pt')
    ...
    prev_img = torch.rand(1, 1, 512, 640)
    cur_img = torch.rand(1, 1, 512, 640)

    input_names = ['input_1', 'input_2']
    output_names = ['output1']
    file_path = os.path.join(path, 'minimum_loss_1ch.onnx')
    torch.onnx.export(model, (prev_img, cur_img), file_path, verbose=True, input_names=input_names, output_names=output_names,
                      opset_version=11)
