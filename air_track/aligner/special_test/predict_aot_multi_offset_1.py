...
def process_single_flight(predictor, part, data_dir, align_save_dir, img_read_method, img_size_w, img_size_h,
    ...
    for i in range(1, len(images)):
        ...
        if i % batch_size == 0:
            ...
            batch_results = predictor.batch_predict(batch_prev, batch_cur)  # 需实现 batch_predict
...
def predict_folder_offsets(predictor, part, num_processes=4):
    """主函数：多进程并行处理所有 flight_id"""
    cfg = predictor.cfg
    ...
    with Pool(num_processes) as p:
        process_fn = partial(process_single_flight, predictor, part, data_dir,
                             align_save_dir, img_read_method, img_size_w, img_size_h)
        all_times = list(tqdm(p.imap(process_fn, flight_ids), total=len(flight_ids)))

    # 合并时间统计
    inference_times = [t for sublist in all_times for t in sublist]
    print(f"平均推理时间: {np.mean(inference_times):.2f}s")
